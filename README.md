...
Pentru automatizare, deschide site-ul și mergi la **Settings → Push automation → Generate webhook**. Copiază endpoint-ul și secretul afișat o singură dată în setările webhook ale repository-ului GitHub sau GitLab și selectează evenimentul **Push**. Endpoint-ul verifică semnătura HMAC/token, limitează payload-ul la 1 MB, respinge replay-urile și acceptă doar repository-ul și branch-ul configurate pentru site. Dacă sosesc mai multe push-uri cât timp un deploy rulează, politica **latest** marchează commiturile încă neîncepute ca `superseded` și execută ultimul commit disponibil. Nu sunt logate payload-uri complete sau secrete; verificările și deploy-urile automate apar în **Activity**.
